...
def get_when_all_presence(driver: WebDriver, locator, wait_seconds=1):
    """Return WebElements by locator when all of them are presence.
    Args:
...
def get_when_number_of_elements_to_be(driver: WebDriver, locator, number=0, wait_seconds=1):
    return WebDriverWait(
        driver,
        wait_seconds) \
        .until(elements_length_changes(locator, number))